...
abstract class BackupModel
{
    ...
    public static function hasOne($p1,$foreign="default",$primary="id")
    {
        ...
        else
        {
            ...
            if($data1==NULL)
            {
                $data1["message"] = "Cannot find any data in Table";
                $data1["status"] = "500"; 
            }
        }
        return json_encode($data1,JSON_NUMERIC_CHECK);
    }
    ...
    public static function select($p1)
    {
        ...
        self::$select.="SELECT $table.$p1 from $table";
        return new static;
    }
    ...
    public static function execute()
    {
        ...
        $sql1=self::$select.self::$combined.';';

        // $sql1="SELECT users.name,users.id,users.email,JSON_ARRAYAGG(JSON_OBJECT('id',students.id,'name',students.name)) as table2 from users LEFT JOIN students ON users.id=students.users_id Group BY users.id having name='sim' Order BY id desc";
        $query1=mysqli_query(self::connection(), $sql1);
        // return $sql1;
        if(!$query1)
        {
